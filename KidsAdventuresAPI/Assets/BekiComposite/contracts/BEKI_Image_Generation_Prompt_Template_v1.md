...
4. A refused base spends the spread's single regeneration (the same budget the review ladder spends). A regenerated base that is refused again stops the spread with `IMAGE_GENERATION_FAILED`, naming both readings — **and carrying the refused picture as failure evidence**, because the first live refusal left nothing behind to judge the gate by.
