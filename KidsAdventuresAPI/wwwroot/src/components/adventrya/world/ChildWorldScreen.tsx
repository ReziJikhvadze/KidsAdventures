...
export function ChildWorldScreen({ celebrationBookId }: ChildWorldScreenProps) {
  ...
  */
  ...
  /*
    ...
    up the same book rather than paying for a second one — but nothing outside `/create` ever
    looked at it, so walking away was a one-way door. Read once on mount: this is a signpost,
    and the screen it points at owns the polling.
  */
  const [pendingRunId] = useState<string | null>(() => readPendingRun()?.runId ?? null);
