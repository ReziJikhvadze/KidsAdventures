...
export const journey = {
  ...
  checkout: {
    ...
    addressNotes: "Extra note",
    requiredRecipient: "Enter the recipient's name.",
    ...
    deliveryDays: (days: number) => `In ${days} working days`,
    deliveryDaysRange: (min: number, max: number) => `In ${min}-${max} working days`,
    deliveryFree: "Free",
    ...
    phoneCheckHeading: "Confirm the recipient's number",
    phoneCheckWhy: "The courier rings this number, so let us make sure it works - once.",
    phoneCheckSent: (destination: string) => `We sent a code to ${destination}`,
    phoneCheckSend: "Send the code",
    phoneCheckResend: "Send it again",
    phoneCheckResendIn: (seconds: number) => ` · ${seconds}s`,
    phoneCheckDigitAria: (index: number) => `Code digit ${index}`,
    phoneCheckDone: "Number confirmed",
    phoneCheckDevCode: (code: string) => `Test code: ${code}`,
    phoneCheckFailed: "The code could not be sent. Try again.",
    phoneCheckPayHint: "Payment carries on once the number is confirmed.",
