...
const draft = () => ({
  worldId: "space", preview: { storyId: "preview-1", coverRevisionId: "revision-1" },
  characters: [{ isPrimary: true, name: "ნინა", birthDate: "2021-01-01", gender: "girl", photoDataUrl: "photo-1" }],
}) as JourneyDraft;
...
for (const patch of [{ birthDate: "2020-01-01" }, { gender: "boy" }, { photoDataUrl: "photo-2" }]) {
  test(`visual input change ${Object.keys(patch)} requires a fresh cover`, () => {
    const before = draft();
    const result = invalidateChangedPreview(before, { ...before, characters: [{ ...before.characters[0], ...patch }] } as JourneyDraft);
    assert.equal(result.preview, null); assert.equal(result.reusePreviewId, undefined);
  });
}
...
test("saving a new hero at checkout preserves the displayed preview", () => {
  const before = draft();
  const result = invalidateChangedPreview(before, { ...before, characters: [{ ...before.characters[0], serverId: "saved" }] });
...
test("a photo edit after a pending rename clears base reuse", () => {
  ...
  const result = invalidateChangedPreview(before, { ...before, characters: [{ ...before.characters[0], photoDataUrl: "new" }] });
